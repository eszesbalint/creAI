from creAI.Tilemaps import ID, Tile, Tilemap
import numpy as np
import creAI.Tilemaps.MinecraftTilemaps.NBT as nbt
import os
import json
import math

class MinecraftID(ID):
    def value_type(self, value):
        if not isinstance(value, tuple):
            raise TypeError("MinecraftID expects a tuple!")
        elif len(value) != 2:
            raise TypeError("MinecraftID expects a tuple of length 2!")
        elif not isinstance(value[0], str) or not isinstance(value[1], str):
            raise TypeError("MinecraftID expects a tuple of 2 strings!")

        return value

    def __str__(self):
        return "{}{}".format(self.value[0], "[{}]".format(self.value[1]) if self.value[1] != "" else "")

class MinecraftTile(Tile):
    def id_object_type(self, id_object):
        if not isinstance(id_object, MinecraftID):
            raise TypeError("MinecraftTile ID is not type StringID!")
        return id_object

class MinecraftTilemap(Tilemap):
    @classmethod
    ...
    def tile_type(cls, tile):
        if not isinstance(tile, MinecraftTile):
            raise TypeError("Array elements are not type MinecraftTile!")
        return tile
...
class Schematic(object):
    @classmethod
    def load(cls, file_name=None, raw_data=None):
        import time
        ...
        for element in root_tag["Palette"].payload:
            ...
            palette[element.payload] = MinecraftTile(MinecraftID((name, variant)))
        start = time.time() ############################
        for block in range(h*l*w):
            ...
            if numeric_id > root_tag["PaletteMax"].payload:
                raise ValueError("Palette ID {} at {} is bigger than PaletteMax {}".format(numeric_id, (x,y,z), root_tag["PaletteMax"].payload))
            
            numeric_ids[x,y,z] = numeric_id 
        end = time.time() 
        print(end - start) 
        tiles = np.vectorize(palette.__getitem__)(numeric_ids)
        return MinecraftTilemap(tiles)

    @classmethod
    def load_as_nbt(cls, file_name=None, raw_data=None):
        return nbt.load(file_name, raw_data)

    @classmethod
    def save(cls, tile_map, path):
        #Swapping axes
        ...
        for tile in tile_set:
            int_tag = nbt.TAG_Int(name=str(tile), payload=numeric_id)
            palette_payload.append(int_tag)
            numeric_id += 1

        palette_tag.payload = palette_payload
        #Creating PaletteMax tag
        palette_max_tag = nbt.TAG_Int(name='PaletteMax', payload=numeric_id)
        #Creating Width tag
        width_tag = nbt.TAG_Short(name='Width', payload=tile_map.shape[0])
        #Creating Height tag
        height_tag = nbt.TAG_Short(name='Height', payload=tile_map.shape[1])
        #Creating Length tag
        length_tag = nbt.TAG_Short(name='Length', payload=tile_map.shape[2])

        #Creating Version tag
        version_tag = nbt.TAG_Int(name='Version', payload=2)

        #Creating DataVersion tag
        data_version_tag = nbt.TAG_Int(name='DataVersion', payload=1976)
        ...
        for tile in swapped_tile_map.flat:
            ...
            for i in range(id_length):
                next_byte = (numeric_id >> i*7) - ( (numeric_id >> (i+1)*7) << (i+1)*7 )
        ...
        root_tag.payload = [palette_tag, palette_max_tag, width_tag, height_tag, length_tag, block_data_tag, version_tag, data_version_tag]
