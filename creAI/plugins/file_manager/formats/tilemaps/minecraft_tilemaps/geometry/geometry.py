...
from creAI.plugins.file_manager.formats.tilemaps.minecraft_tilemaps import (Minecraft_ID, Minecraft_Tile,
                                                                            Minecraft_Tilemap)
...
def get_model_json(tile: Minecraft_Tile) -> dict:
    name = tile.id_object.value[0].split(':')[1]
    variant = tile.id_object.value[1]
    json_data = open(
        os.path.join(
            blockstates_folder_path,
            name
            + '.json'
        )
    ).read()
...
def tile_to_geometry(tile: Minecraft_Tile) -> np.ndarray:
...
def tilemap_to_geometry(tilemap: Minecraft_Tilemap) -> np.ndarray:
