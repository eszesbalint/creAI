...
def generate(width: int, height: int, length: int) -> Minecraft_Tilemap:
    ...
    tilemap = np.empty((width, height, length), dtype=object)
    tilemap[:, :, :] = Minecraft_Tile(Minecraft_ID(('minecraft:air', '')))

    symbols = [Symbol('start', 0, 0, 0, tilemap.shape[0],
                      tilemap.shape[1], tilemap.shape[2])]
    new_symbols = []
    removed_symbols = []
    ...
    def rule1():
        ...
        removed_symbols += [s]
        free_space = Symbol('free_space_1', s.x1, s.y1, s.z1,
                            s.x2, s.y2, s.z2, axis='x', level=0)
    ...
    def rule2():
        ...
        if (s.x2-s.x1 > 16) and (s.z2-s.z1 > 16):
            if s.property['axis'] == 'x':
                ...
                z2 = z1 + 16
                x2 = s.x2
                floor_plan = Symbol('floor_plan', x1, 0, z1,
                                    x2, 0, z2, height=(10-s.property['level'])*4)
                new_symbols += [floor_plan]
                floor_plans += [floor_plan]
                free_space1 = Symbol('free_space', s.x1+16, 0, s.z1,
                                     x2-16, 0, z1, axis='z', level=s.property['level']+1)
                free_space2 = Symbol(
                    'free_space', x1+16, 0, z2, s.x2-16, 0, s.z2, axis='z', level=s.property['level']+1)
            ...
            if s.property['axis'] == 'z':
                ...
                floor_plans += [floor_plan]
                new_symbols += [floor_plan]
                free_space1 = Symbol('free_space', s.x1, 0, s.z1+16,
                                     x1, 0, z2-16, axis='x', level=s.property['level']+1)
                free_space2 = Symbol('free_space', x2, 0, z1+16, s.x2,
                                     0, s.z2-16, axis='x', level=s.property['level']+1)
                new_symbols += [free_space1, free_space2]
    ...
    def rule4():
        ...
        middle_height = bottom_height + \
            ((s.property['height']-bottom_height)//4)*4
        middle = Symbol('middle', s.x1, bottom_height,
                        s.z1, s.x2, middle_height, s.z2)
    ...
    def rule5():
        ...
        if s.x2-s.x1 > s.z2-s.z1:
            facade1 = Symbol('facade', s.x1, s.y1, s.z1 - 1,
                             s.x2, s.y2, s.z1, axis='z', facing='-')
            facade2 = Symbol('facade', s.x1, s.y1, s.z2, s.x2,
                             s.y2, s.z2 + 1, axis='z', facing='+')
        else:
            facade1 = Symbol('facade', s.x1 - 1, s.y1, s.z1,
                             s.x1, s.y2, s.z2, axis='x', facing='-')
            facade2 = Symbol('facade', s.x2, s.y1, s.z1, s.x2 +
                             1, s.y2, s.z2, axis='x', facing='+')
    ...
    def rule6():
        ...
        wall_strips = ['large_window_strip', 'balcony_strip',
                       'small_window_strip', 'vent_strip']
        wall_strip_widths = [5, 5, 4, 3]
        pattern = [random.choice(list(zip(wall_strips, wall_strip_widths)))
        ...
        if s.property['axis'] == 'z':
            ...
            while True:
                ...
                x1 = s.x1 + delta
                x2 = s.x1 + delta + width
                strip = Symbol(name, x1, s.y1, s.z1, x2, s.y2,
                               s.z2, axis='z', facing=s.property['facing'])
            ...
            while True:
                ...
                z1 = s.z1 + delta
                z2 = s.z1 + delta + width
                strip = Symbol(name, s.x1, s.y1, z1, s.x2, s.y2,
                               z2, axis='x', facing=s.property['facing'])
    ...
    def rule7():
        ...
        element = elements[s.name[:-6]
                           ][s.property['facing']+s.property['axis']]
    ...
    def rule8():
        ...
        tilemap[s.x1:s.x2, s.y1:s.y2, s.z1:s.z2] = Minecraft_Tile(
